...
do
  echo $appKey
  stmt=`echo curl -v -X POST \"$eventsURL/events/query\" -H\"X-Events-API-AccountName:$API_ACCOUNT\" -H\"X-Events-API-Key:$API_KEY\" -H\"Content-type: application/vnd.appd.events+text\;v=2\" -d \'SELECT pagename, pageurl FROM browser_records WHERE appkey = \"ABCDEF\" SINCE 30 minutes\' | sed "s/ABCDEF/$appKey/g"`
  eval $stmt > ./tmpFile.json
  entriesFound=0;
   cmd="python3 ./formatJson.py | grep -E '(?:[0-9]{1,3}\.){3}[0-9]{1,3}|[0-9]{16}|[0-9]{16}|(\.[\w\-]+)*@([A-Za-z0-9-]+\.)+[A-Za-z]{2,4}' $OPTIONS |sed -E 's/\b\w+@/###  Email-Address  ### @/g' | sed -E 's/[0-9]{16}/#### 16 Digits ####/g' | sed -E 's/\"//g'"
  ...
  if [[ $entriesFound -eq 0 ]]; then
     ...
     Description="Healthy"
     Data=""
     echo  curl -k  -X POST "$eventsURL/events/publish/INTERNAL_AUDITS" -H"X-Events-API-AccountName:$API_ACCOUNT" -H"X-Events-API-Key:$API_KEY" -H"Content-type: application/vnd.appd.events+json;v=2" -d "[ { \"Environment\": \"$1\", \"Tenant\": \"$appKey\", \"SnapshotType\": \"Browser Record\", \"AuditTime\": \"$NOW\", \"Status\": \"$Status\", \"Description\": \"$Description\", \"SampleData\": \"$Data\" } ]"
     curl -k  -X POST "$eventsURL/events/publish/INTERNAL_AUDITS" -H"X-Events-API-AccountName:$API_ACCOUNT" -H"X-Events-API-Key:$API_KEY" -H"Content-type: application/vnd.appd.events+json;v=2" -d "[ { \"Environment\": \"$1\", \"Tenant\": \"$appKey\", \"SnapshotType\": \"Browser Record\", \"AuditTime\": \"$NOW\", \"Status\": \"$Status\", \"Description\": \"$Description\", \"SampleData\": \"$Data\" } ]"
  else
