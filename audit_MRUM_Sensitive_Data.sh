...
if [ "$env" == "Prod" ]; then
    controllerURL="https://ddddsaas.appdynamics.com/controller"
    hashedCredentials='xxxxM='
    API_ACCOUNT="xxxx32f"                                  # Controller Global Account Name
    API_KEY="xxxxxdb00368c50" 
    ProxyServer='XXXX'
    ProxyPort='XXXX'
else
  if [ "$env" == "Test" ]; then
...
do
  ...
  URL="https://analytics.api.appdynamics.com/events"
   if [[ $1 == "Prod" ]]; then
    ...
    AccountName="xxxx45f9-8f1f-20875e4ac32f"                                  # Controller Global Account Name
    API_KEY="xxxxxabc-75db00368c50" 
